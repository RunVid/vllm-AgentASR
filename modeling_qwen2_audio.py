...
class Qwen2AudioEncoder(Qwen2AudioPreTrainedModel):
    ...
    def forward(
        ...
        # When batch_size==1, trim padded timesteps before convolutions to save compute
        # buffer_tokens: extra tokens to avoid conv boundary effects (conv2 kernel=3, stride=2)
        buffer_tokens = 3
        if remove_padding and attention_mask is not None and batch_size == 1 and attention_mask.dim() == 4:
            valid_seq_len = int(torch.isfinite(attention_mask[0, 0, 0]).sum().item())
            if valid_seq_len > 0:
                # Compute target_seq_len first, then derive target_mel_len to ensure strict 2:1 ratio
                # Formula: seq_len = (mel_len - 2) // 2 + 1, so mel_len = 2 * seq_len
                target_seq_len = min(attention_mask.shape[-1], valid_seq_len + buffer_tokens)
                target_mel_len = min(input_features.shape[-1], 2 * target_seq_len)
                # Log for debugging: compare encoder trimmed length vs actual needed length
                # After pooler, output length = (target_seq_len - 2) // 2 + 1
                encoder_output_len_after_trim = (target_seq_len - 2) // 2 + 1
                print(f"[Encoder remove_padding] valid_seq_len={valid_seq_len}, buffer={buffer_tokens}, "
                      f"target_seq_len={target_seq_len}, target_mel_len={target_mel_len}, "
                      f"encoder_output_len_after_trim={encoder_output_len_after_trim}")
                if target_mel_len < input_features.shape[-1]:
                    input_features = input_features[:, :, :target_mel_len]
                    attention_mask = attention_mask[:, :, :target_seq_len, :target_seq_len]
        ...
        if remove_padding and attention_mask is not None and batch_size == 1 and attention_mask.dim() == 4:
            ...
            # Dynamic slicing of positional embeddings based on actual sequence length
        ...
        #print(f"[Qwen2AudioEncoder] hidden_states shape: {hidden_states.shape}")
        #print(f"[Qwen2AudioEncoder] attention_mask shape: {attention_mask.shape}")
